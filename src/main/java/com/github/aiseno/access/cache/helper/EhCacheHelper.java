...
public class EhCacheHelper implements ICache {
	...
	public boolean cleanContext() {
		...
		log.info("access cache size = {} , clean = {} " , size,size);
		return removeAll();
	}
	...
	private static class DefaultCache {
		...
		static {
			...
			cacheConfiguration.setTimeToIdleSeconds(7200L);//缓存创建以后，最后一次访问缓存的日期至失效之时的时间间隔(秒) 2 小时
			cacheConfiguration.setDiskExpiryThreadIntervalSeconds(600);//磁盘清理线程的运行时间间隔(秒) 十分钟
			cacheConfiguration.setDiskSpoolBufferSizeMB(60);//大小
